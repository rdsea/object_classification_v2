...
from rohe_ml_agent import RoheMLAgent


class ObjectClassificationAgent(RoheMLAgent):
    def __init__(
        self,
        load_model_params: dict,
        model_id: str,
        input_shape: tuple = (32, 32, 3),
        load_model_from_file_mode=True,
    ):
        self.load_model_from_file_mode = load_model_from_file_mode
        if isinstance(input_shape, str):
            input_shape = rohe_utils.convert_str_to_tuple(input_shape)

        self.load_model_params: dict = self._structure_load_model_params(
            load_model_params
        )

        super().__init__(
            load_model_params=self.load_model_params,
            model_id=model_id,
    ...
    def load_model(self, load_model_params):
        ...
        if self.load_model_from_file_mode:
            ...
            raise ValueError(
                "Now, only support load model from config files, not support loading model defined from code."
            )
    ...
    def _construct_prediction_result(self, image: np.ndarray) -> dict:
        ...
        result = {
            "class": int(predicted_class_index),
    ...
    def _structure_load_model_params(self, load_model_params: dict) -> dict:
        ...
        for model_id, folder in models.items():
            ...
            _folder = current_dir_path + folder
            model_params[model_id] = {
                "files": {
                    "architecture_file": os.path.join(_folder, architecture_file),
                    "weights_file": os.path.join(_folder, weights_file),
                }
            }

        return model_params

    def _load_model_from_file(self, architecture_file: str, weights_file: str):
